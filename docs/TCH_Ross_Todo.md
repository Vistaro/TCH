...
| # | Item | Notes |
|---|------|-------|
| 23 | **🔴 START-OF-SESSION — PROD ship v0.9.26 (Phases A–D from 2026-04-20)** | Added 2026-04-21. On `dev` at `9f50430`, not yet on PROD. Ships: (a) migrate.sh retention fix, (b) caregiver loan ledger (mig 047 + `/admin/caregiver-loans`), (c) Nominatim geocoding (`includes/geocode.php` + auto-hook on patient save + `/admin/dev-tools/geocode-backfill`), (d) patient care-needs profile + emergency contacts (mig 048 + two new card sections on patient detail). **All super_admin-only on PROD after ship — Tuniti sees nothing new** (release-gating intact). **When Ross says "ship":** same 8-step chain as v0.9.25, mig files 047 + 048 only. Write up in CHANGELOG under `[v0.9.26]` entry, ff-merge dev→main, rsync to PROD, tag. **Pre-ship sanity:** verify 039–046 are all on PROD (one-command runner check); don't re-run them. |
| 22 | **🔴 FIRST-ACTION-NEXT-SESSION — Forth malware triage** | Added 2026-04-21, updated 2026-04-21 wrap. Governance confirmed the 2 flagged paths are TCH's own `db-backups/tch/prod-webroot-pre-0920-20260413-160734.tgz` + `...pre-0917-20260413-105958.tgz` archives. Scanner hits a PHP file inside with webshell-shape signature (cookie + user-agent + file_get_contents + tempnam + file_put_contents + unlink). Almost certainly legitimate `includes/onboarding_upload.php` tripped as false positive. **Next-session Agent action:** SSH in → extract one archive to scratch folder → grep for 6-token signature → open candidates → verify each token has legitimate reason → draft false-positive explanation for Ross to send Forth support → audit backup-folder retention policy. Full guidance in `_global/output/agent-messages/2026-04-21-1100-governance-to-tch-forth-flagged-files-are-your-backup-archives.md`. Urgency: PHP mail off account-wide. |
| 20 | **Quote builder: "Save & Send" one-click button — review with Tuniti input** | Added 2026-04-18. **Context:** the current quote builder at `/admin/quotes/new` saves as Draft. Marking Sent is a separate click on the quote detail page (the status-transition action bar, "Mark as Sent" button). So the end-to-end flow is: build → save → land on detail → click Send → done. That's two explicit confirmations, which may be deliberate (review before emailing) or unnecessary friction. **Proposed change:** add a "Save & Send" button next to "Save changes" on the builder that saves and stamps `status='sent'` + `sent_at=NOW()` in one go. **Why parked:** depends on how Tuniti actually works — if she wants a "review one last time before sending" moment, two clicks is right. If she's building the quote in front of the customer, one click is better. **Decision input needed:** Tuniti's actual workflow preference once she's used the builder a few times. **Implementation sketch when approved:** add a second submit button `<button name="and_send" value="1">` in the form, detect it in the POST handler, and on create-with-and-send flip status to `'sent'` + stamp `sent_at`. Re-use the existing send validation logic from `quotes_detail.php` action=send. Est. 15 minutes. |
| 21 | **Opportunity detail: hide "backward" stage-transition buttons?** | Added 2026-04-18. **Context:** on `templates/admin/opportunities_detail.php`, the stage-transition button bar renders one button for every stage that isn't the current one. For an opp currently in "Quoted", that means buttons for New / Qualifying / Negotiating / Closed-Won / Closed-Lost all appear — including the two "backward" moves (→ New, → Qualifying). Ross raised in the 2026-04-18 cleanup review that backward moves clutter the UI with counter-intuitive options. **Options:** (a) leave as-is — explicit freedom, nothing hidden; (b) hide stages with `sort_order < current.sort_order` from the bar — backward moves only reachable by editing the opp; (c) show them dimmed and requiring confirm() — explicit "are you sure you want to go backward?" gate. **Decision pending:** Ross to review usage once the Kanban sees real use. **Implementation sketch if (b) chosen:** single predicate in the `foreach ($stages as $s)` loop that skips when `$s['sort_order'] < $opp['sort_order']`. Kanban already implicitly forbids backward drags in many cases (dragging a closed card is moot because it's off the board). Est. 5 minutes. |
| 19 | **Sales pipeline (FR-L) + lifecycle reporting (FR-M)** | Added 2026-04-18 after Ross's steer that the quote builder must sit inside a sales-pipeline context. **FR-L:** opportunities table + Kanban pipeline view (Nexus CRM pattern). Enquiries convert to opportunities; quotes attach to opportunities not enquiries; stages run New → Qualifying → Quoted → Negotiating → Closed-Won/Lost. **FR-M:** three reporting surfaces — **Acquire** (pipeline KPIs: open value by stage, conversion rate, win rate, forecasts), **Manage** (ops KPIs: active patients, hours delivered, revenue, margin, caregiver continuity), **Exit** (strategic KPIs: churn, reason-for-leaving breakdown, length-of-relationship). Full writeups in `docs/TCH_Quote_And_Portal_Plan.md` §4. Ross's guidance: FR-L should ship **before or alongside FR-C** so the quote builder stamps `contracts.opportunity_id` from day 1 — avoids a later backfill. |
...
| 16 | ~~**Client/patient onboarding workflow — care proposal + email acceptance, blocks scheduling until accepted**~~ | **SUPERSEDED 2026-04-16** by the Quote & Portal Plan in `docs/TCH_Quote_And_Portal_Plan.md`. Same scope (draft proposal/quote → product lines → email with tokenised link → client accepts → contract active → blocks scheduling until accepted) expressed as a cleaner 11-FR series. Work now tracked as: **FR-C** (quote builder), **FR-D** (state machine + admin-triggered acceptance), **FR-G** (email delivery + client preference), **FR-H** (Client/Patient user classes + invite-on-accept), **FR-I** (portal tokenised acceptance). Phase-1 engagements.php hard-block on `patients.client_id IS NOT NULL` already shipped; the schedule-guardrail extension (status='accepted' required before engagement create) becomes part of FR-D's acceptance transition. Do not build this item as a separate track — follow the Quote & Portal Plan. |
...
| TODO-devdb-sanitise | **Write `scripts/dev-db-sanitise.sql` + run first DEV refresh from PROD** | HIGH | First live exercise of the post-ship DEV-refresh rule in the new project-local `CLAUDE.md`. Minimum ruleset per global standing order: names → fakes, emails → `user{id}@dev.invalid`, phones → `+44-DEV-xxxx`, DOBs ±30d shift, payment/API/session tokens nulled, free-text PII truncated. TCH-specific: GDPR Art. 9 sweep across `patient_care_needs` TEXT fields since mig 048 — medical/allergies/medications/DNR/cognitive/etc. all count as special-category data. Preserve rows with `is_test_data = 1`. Once script exists: snapshot PROD → restore to DEV → apply sanitise → verify. ~30-45 min once script is written; picking up next session per Ross's (c) call 2026-04-21 post-v0.9.26 ship. |
| TODO-deploy-ssh-key-path | **`scripts/deploy.sh` — allow SSH key path override** | MEDIUM | Currently hard-codes `$HOME/.ssh/intelligentae_deploy_ed25519` which broke mid-ship on 2026-04-21 when the desktop had the key only at `_global/keys/`. Workaround was copying the key into `~/.ssh/`. Durable fix: support `DEPLOY_SSH_KEY` env var override, or check `_global/keys/` as a fallback. Same pattern likely needed on any future deploy helpers. ~10 min. |
