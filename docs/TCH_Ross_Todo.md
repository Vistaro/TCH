...
| 22 | **🔴 START-OF-SESSION — Forth Host malware paths** | Added 2026-04-21. Governance flagged on `forth-malware-2026-04-20` thread: Forth's scanner flagged 2 infected files on `intelligentae.co.uk` at 2026-04-20 08:13Z. PHP mail is disabled on the whole hosting account (TCH + Nexus-CRM + Nexus-Hub share it). **Bottleneck:** only Ross has `cp.forthhost.com` access. **Ross action:** log in → copy the 2 flagged file paths → paste here. Then TCH agent (or whichever project agent owns the path) triages false-positive vs real injection. Nexus-CRM is urgent (in live use, mail silently failing). TCH is pre-live so dev-time only. |
