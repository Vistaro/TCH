...
| 13 | **🔴 URGENT — Separate DEV and PROD databases before Tuniti UAT** | DEV and PROD currently share `tch_placements-313539d33a` (FR-0076 exception). Tuniti UAT means real test writes from caregivers/clients on DEV — those will pollute PROD billing rows. Do BEFORE the first Tuniti session: (a) create new DB `tch_placements_dev-XXXX` via StackCP panel; (b) `mysqldump` prod → restore to dev DB; (c) update `~/public_html/dev-TCH/dev/.env` → new creds; (d) smoke-test dev hits new DB; (e) update DECISIONS.md + close FR-0076 on Hub. ~30 min once the DB exists. Ross to create the empty DB via StackCP (or grant panel access). |
